...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const getOrCreateUser = useCallback(async (firebaseUser: FirebaseUser): Promise<User | null> => {
    try {
      ...
      // Check if user is a configured test account
      ...
      if (existingUser) {
        ...
        if (testAccountConfig) {
          ...
          if (testAccountConfig.isWhitelisted && !existingUser.isWhitelisted) {
            updates.isWhitelisted = true;
            needsUpdate = true;
          }
          if (testAccountConfig.grade !== undefined && existingUser.grade !== testAccountConfig.grade) {
              // Only update grade if explicitly defined in config
        ...
        const newUser: Omit<User, 'uid'> = {
          ...
          grade: testAccountConfig?.grade !== undefined ? testAccountConfig.grade : null, // Default to null or test config
